...
class DataManager:
    ...
    async def get_text_statistics(self) -> dict:
        ...
        for col in text_columns:
            ...
            if len(text_data) == 0:
                text_stats[col] = {
                    "error": f"No valid data in column '{col}'."
                }
                continue
            ...
            if col == "Subject":
                email_specific.update(
                    {
                        "subjects_with_re": text_data.str.contains(
                            r"^Re:", case=False, na=False
                        ).sum(),
                        "subjects_with_fwd": text_data.str.contains(
                            r"^Fwd:", case=False, na=False
                        ).sum(),
                        "subjects_all_caps": text_data.str.isupper().sum(),
                        "subjects_with_urgent_words": text_data.str.lower()
                        .str.extract(
                ...
                email_specific.update(
                    {
                        ...
                        "avg_sentences": text_data.str.split(
                            r"[.!?]+"
                        ).str.len().mean(),
                        ...
                        base_stats.update(
                            {
                                ...
                                "most_common_domain_count": (
                                    domain_counts.iloc[0]
                                    if len(domain_counts) > 0
                                    else 0
                                ),
                                "single_use_addresses": (
                                    non_null_data.value_counts() == 1
                                ).sum(),
                                "avg_emails_per_address": (
                                    len(non_null_data) / unique_count
                                    if unique_count > 0
                                    else 0
                                ),
                            }
    ...
    async def get_data_quality_report(self) -> dict:
        ...
        if high_missing:
            issues.append(
                f"High missing data (>50%): {', '.join(high_missing)}"
            )
    ...
    async def handle_quality_issues(
        self, drop_constants: bool = True, threshold: float = 50.0
    ):
        """Run preprocessing steps from preprocessing.py."""
    ...
    async def run_feature_engineering(self, top_k_domains: int):
        ...
        pre = Preprocessor(self)
        try:
            feature_cols = await pre.create_text_features(
    ...
    async def run_vectorization(
        ...
        """Run text vectorization using Preprocessor and return the feature matrix."""
        pre = Preprocessor(self)
        try:
