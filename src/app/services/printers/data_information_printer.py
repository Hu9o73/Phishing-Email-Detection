...
class DataInformationPrinter(Printer):
    ...
    def print_basic_info(self):
        ...
        print(
            f"Memory usage: "
            f"{self.format_bytes(basic_info.get('memory_usage', 0) * 1024**2)}"
        )
    ...
    def print_text_statistics(self):
        ...
        for col_name, stats in text_stats.items():
            ...
            print(
                f"\t- Length range: "
                f"{stats['min_char_length']:.0f} - {stats['max_char_length']:.0f} characters"
            )
            print(f"\t- Standard deviation: {stats['std_char_length']:.1f}")
            print(f"\t- Average words: {stats['avg_word_count']:.0f}")
            print(
                f"\t- Word range: "
                f"{stats['min_word_count']:.0f} - {stats['max_word_count']:.0f} words"
            )
            ...
            if col_name == "Subject":
                ...
                print(
                    f"\t- Urgent keywords: "
                    f"{stats.get('subjects_with_urgent_words', 0):,}"
                )
                print(
                    f"\t- Suspicious characters: "
                    f"{stats.get('subjects_with_suspicious_chars', 0):,}"
                )

                urgent_ratio = (
                    stats.get("subjects_with_urgent_words", 0)
                    / stats["total_samples"]
                    * 100
                )
                caps_ratio = (
                    stats.get("subjects_all_caps", 0)
                    / stats["total_samples"]
                    * 100
                )
                if urgent_ratio > 5:
                    print(
                        f"\t/!\\ High urgent keyword usage "
                        f"({urgent_ratio:.1f}%) - potential phishing indicator"
                    )
                if caps_ratio > 3:
                    print(
                        f"\t/!\\ High all-caps usage "
                        f"({caps_ratio:.1f}%) - potential spam/phishing indicator"
                    )
                ...
                print(
                    f"\t- Contains email addresses: "
                    f"{stats.get('contains_email_addresses', 0):,}"
                )
                print(
                    f"\t- Contains phone numbers: "
                    f"{stats.get('contains_phone_numbers', 0):,}"
                )
                print(
                    f"\t- Money-related content: "
                    f"{stats.get('contains_money_symbols', 0):,}"
                )
                print(
                    f"\t- Average sentences: "
                    f"{stats.get('avg_sentences', 0):.1f}"
                )
                print(
                    f"\t- Mentions attachments: "
                    f"{stats.get('contains_attachments_mention', 0):,}"
                )

                url_ratio = (
                    stats.get("contains_urls", 0)
                    / stats["total_samples"]
                    * 100
                )
                money_ratio = (
                    stats.get("contains_money_symbols", 0)
                    / stats["total_samples"]
                    * 100
                )
            ...
            if stat_type.endswith("_stats"):
                ...
                if category.lower() in ["sender", "recipient"]:
                    ...
                    if most_common_domain:
                        print(
                            f"\t- Most common domain: {most_common_domain} "
                            f"({most_common_count:,} emails)"
                        )
                    print(f"\t- Single-use addresses: {single_use:,}")
                    print(
                        f"\t- Average emails per address: {avg_emails:.1f}"
                    )
    ...
    def print_data_quality_report(self):
        ...
        if missing_data_cols:
            ...
            for col, missing_count, completeness_rate in sorted(
                missing_data_cols, key=lambda x: x[2]
            ):
                print(
                    f"\t- {col}: {missing_count:,} missing "
                    f"({100 - completeness_rate:.1f}%)"
                )
